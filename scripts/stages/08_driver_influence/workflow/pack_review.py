...
import tarfile
if not (OUT/"MAR08_PASS.txt").is_file(): raise SystemExit("HOLD: postflight first")
h=MA/"90_handoff"; h.mkdir(parents=True,exist_ok=True); ts=datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"); p=h/f"MAR08_REVIEW_{ts}.tar.gz"
keep=["MAR08_preflight_report.tsv","MAR08_resolved_inputs.json","MAR08_preanalysis_lock.json","MA_MAR08_STOPA_RELEASE.txt","MAR08_FROZEN_SCENARIO_MANIFEST.tsv","MAR08_FROZEN_SCENARIO_COVERAGE.tsv","MAR08_postflight_checks.tsv","MAR08_INFLUENCE_SUMMARY.tsv","MAR08_MANUSCRIPT_DECISION.tsv","MAR08_analysis_lock.json","MAR08_checksums.sha256","MAR08_PASS.txt"]
