...
import sys
if len(sys.argv)<2 or sys.argv[1]!="RELEASE": raise SystemExit("Usage: release.py RELEASE")
p=OUT/"MAR08_preanalysis_lock.json"
if not p.is_file(): raise SystemExit("HOLD: freeze first")
(OUT/"MA_MAR08_STOPA_RELEASE.txt").write_text(
 f"decision=PASS\ntrack=MOLECULAR_AUTISM_REVISION\nstage=MAR08\ngate=A\nreviewer=MANUAL_EXPERT_REVIEW\ntimestamp_utc={utc()}\npreanalysis_lock_sha256={sha256_file(p)}\nnotes=Revised MAR08 diagnostic contract released; MAR07 FAIL remains binding; no rescue analysis permitted.\n",encoding="utf-8")
print("MAR08_GATE_A=PASS")
