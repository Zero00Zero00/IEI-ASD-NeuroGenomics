...
ensure_dirs(); root=MAR07_RUN/"executor_work"; side=[root/f"rep_{i:04d}"/"mdv3.tsv" for i in range(1,1001)]; present=sum(p.is_file() for p in side)
notrun=OUT/"analysis"/"MAR08_NULL_PATHWAY_RECURRENCE_NOT_RUN.txt"; out=OUT/"analysis"/"MAR08_NULL_PATHWAY_RECURRENCE.tsv"
if present!=1000:
    notrun.write_text(f"status=NOT_RUN_WITH_REASON_PATHWAY_LEVEL_NULL_STATE_NOT_FROZEN\nmdv3_sidecars_present={present}/1000\nreason=Contract permits pathway recurrence only from complete pre-existing 1000/1000 sidecars; no recomputation allowed.\nTier1_pathway_recurrence=NOT_RUN_WITH_REASON_PATHWAY_LEVEL_NULL_STATE_NOT_FROZEN\n",encoding="utf-8")
    print(f"MAR08_MODULE_F=NOT_RUN sidecars={present}/1000")
    raise SystemExit(0)
counts=collections.Counter(); names={}; keyset=None
for i,p in enumerate(side,1):
    h,rows=read_tsv(p); cur=set()
    for z in rows:
        key=(z["source"],z["pathway_id"]); cur.add(key); names[key]=z.get("pathway_name","")
        if str(z.get("enriched_flag","")).strip().upper() in {"TRUE","1","T","YES"}: counts[key]+=1
    if len(cur)!=6671: raise SystemExit(f"HOLD: rep {i} mdv3 rows/keys !=6671")
    if keyset is None:keyset=cur
    elif cur!=keyset: raise SystemExit(f"HOLD: rep {i} pathway universe differs")
rows=[{"stage":"MDV3","source":k[0],"pathway_id":k[1],"pathway_name":names.get(k,""),"R":1000,"positive_n":counts[k],"positive_prop":counts[k]/1000,"interpretation":"CONDITIONAL_SUPPORTIVE_MD3_ONLY"} for k in sorted(keyset)]
write_tsv(out,list(rows[0].keys()),rows)
(OUT/"analysis"/"MAR08_NULL_TIER1_RECURRENCE_NOT_RUN.txt").write_text("status=NOT_RUN_WITH_REASON_PATHWAY_LEVEL_NULL_STATE_NOT_FROZEN\nreason=MAR07 executor did not persist complete per-pathway MDV4/Tier1 state; contract prohibits rerunning 1000 full-chain replicates solely to recover identities.\n",encoding="utf-8")
print("MAR08_MODULE_F_MDV3_RECURRENCE=PASS"); print("MAR08_MODULE_F_TIER1_RECURRENCE=NOT_RUN")
