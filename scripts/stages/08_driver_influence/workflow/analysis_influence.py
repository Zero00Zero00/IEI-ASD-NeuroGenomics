...
if not tok.is_file() or parse_token(tok).get("preanalysis_lock_sha256")!=sha256_file(lockp): raise SystemExit("HOLD: valid release required")
resolved=json.loads((OUT/"MAR08_resolved_inputs.json").read_text()); sys.path.insert(0,str(PKG/"vendor")); import mar03_common as m3
magma=Path(resolved["MAGMA_BIN"]); _,manifest=read_tsv(OUT/"MAR08_FROZEN_SCENARIO_MANIFEST.tsv"); _,cov=read_tsv(OUT/"MAR08_FROZEN_SCENARIO_COVERAGE.tsv"); cv={(z["dataset"],z["scenario_id"],z["target_id"]):z for z in cov}
_,base=read_tsv(Path(resolved["MAR03_PRIMARY"])); bm={(z["dataset"],z["target_id"]):z for z in base}; targets=sorted({z["target_id"] for z in base})
def run_one(ds,s):
    sdir=OUT/"work"/"magma"/ds/s["scenario_id"]; sdir.mkdir(parents=True,exist_ok=True); pref=sdir/"marginal"; setf=OUT/"work"/"sets"/ds/f"{s['scenario_id']}.sets"; raw=Path(resolved[f"{ds}_W10_RAW"])
    cmd=[magma,"--gene-results",raw,"--set-annot",setf,"--model","direction=greater","--out",pref]
    m3.run_cmd(cmd,str(pref)+".wrapper.log"); _,r=m3.parse_gsa(m3.gsa_output(pref)); out=[]
    for t in targets:
        rr=m3.get_result_row(r,t); ng,beta,bstd,se,p=m3.result_values(rr); c=cv[(ds,s["scenario_id"],t)]
        out.append({"dataset":ds,"scenario_type":s["scenario_type"],"scenario_id":s["scenario_id"],"omitted_genes":s["omitted_genes"],"origin_units":s["origin_units"],"target_id":t,"affected_by_omission":c["affected_by_omission"],"NGENES":ng,"beta":beta,"BETA_STD":bstd,"SE":se,"P_one":p,"q_BH":""})
    qs=bh([z["P_one"] for z in out])
    for z,q in zip(out,qs): z["q_BH"]=q
    return out
# baseline replay first
replay=[]
for ds in ["PGC","SPARK"]:
    s=next(z for z in manifest if z["scenario_id"]=="BASELINE"); rr=run_one(ds,s)
    for z in rr:
        b=bm[(ds,z["target_id"])]; deltas={f"delta_{f}":abs(float(z[f])-float(b[f])) for f in ["beta","SE","P_one","q_BH"]}; ok=all(v<=1e-8 for v in deltas.values())
        replay.append({"dataset":ds,"target_id":z["target_id"],"status":"PASS" if ok else "FAIL",**deltas})
write_tsv(OUT/"analysis"/"MAR08_BASELINE_REPLAY.tsv",list(replay[0].keys()),replay)
if any(z["status"]!="PASS" for z in replay): raise SystemExit("HOLD: MAR03 baseline replay failed")
# scenarios
l1=[]; l2=[]
for s in manifest:
    if s["scenario_type"] not in {"LEAVE_ONE","LEAVE_TWO"}: continue
    for ds in ["PGC","SPARK"]:
        rr=run_one(ds,s)
        for z in rr:
            b=bm[(ds,z["target_id"])]; base_pos=float(b["beta"])>0 and float(b["q_BH"])<0.05; now_pos=float(z["beta"])>0 and float(z["q_BH"])<0.05
            z.update({"baseline_beta":b["beta"],"baseline_SE":b["SE"],"baseline_P_one":b["P_one"],"baseline_q_BH":b["q_BH"],"delta_beta_vs_frozen":float(z["beta"])-float(b["beta"]),"sign_flip":"YES" if sign(z["beta"])!=sign(b["beta"]) else "NO","classification_switch":"YES" if now_pos!=base_pos else "NO","scenario_primary_positive":"YES" if now_pos else "NO","interpretation":"SENSITIVITY_ONLY_NOT_RESCUE" if now_pos else "DIAGNOSTIC_NULL"})
            (l1 if s["scenario_type"]=="LEAVE_ONE" else l2).append(z)
fields=list(l1[0].keys()); write_tsv(OUT/"analysis"/"MAR08_LEAVE_ONE_DRIVER.tsv",fields,l1)
fields2=list(l2[0].keys()); write_tsv(OUT/"analysis"/"MAR08_LEAVE_TWO_DRIVER.tsv.gz",fields2,l2)
print("MAR08_MODULES_D_E=PASS"); print(f"LEAVE_ONE_ROWS={len(l1)}"); print(f"LEAVE_TWO_ROWS={len(l2)}")
