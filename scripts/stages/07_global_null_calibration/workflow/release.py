...
p=OUT/"MAR07_preanalysis_lock.json"
if not p.is_file(): raise SystemExit("HOLD: freeze first")
(OUT/"MAR07_STOPA_RELEASE.txt").write_text(
    "decision=PASS\ntrack=MOLECULAR_AUTISM_REVISION\nstage=MAR07_R4\ngate=A\n"
    f"timestamp_utc={utc()}\npreanalysis_lock_sha256={sha256_file(p)}\n"
    "notes=Replay certification and full-chain null design frozen before execution.\n",
    encoding="utf-8")
print("MAR07_R4_GATE_A=PASS")
