...
ensure_dirs()
for src,dst in [
 (R3/"observed"/"MAR07_bh_by_sensitivity.tsv",OUT/"observed"/"MAR07_bh_by_sensitivity.tsv"),
 (R3/"observed"/"MAR07_observed_tier1_stability.tsv",OUT/"observed"/"MAR07_observed_tier1_stability.tsv"),
 (R3/"observed"/"MAR07_observed_dependency_summary.tsv",OUT/"observed"/"MAR07_observed_dependency_summary.tsv")]:
    if not src.is_file(): raise SystemExit(f"HOLD: missing R3 observed {src}")
    shutil.copy2(src,dst)
_,tiers=read_tsv(OUT/"observed"/"MAR07_observed_tier1_stability.tsv")
bh=sum(z["Tier1_BH"]=="YES" for z in tiers)
by=sum(z["Tier1_BY"]=="YES" for z in tiers)
if (bh,by)!=(39,0): raise SystemExit(f"HOLD: observed oracle mismatch {(bh,by)} expected (39,0)")
print("MAR07_R4_OBSERVED_IMPORT=PASS")
